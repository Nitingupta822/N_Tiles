...
# Use environment variable for secret key, generate random one for development
secret_key = os.environ.get('SECRET_KEY')
if not secret_key:
    secret_key = secrets.token_hex(32)
    if os.environ.get('FLASK_ENV') == 'production':
        print("WARNING: SECRET_KEY not set in production! Using temporary key.")
        print("Please set SECRET_KEY environment variable.")
    else:
        print(f"Development mode: Using generated secret key: {secret_key}")
...
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(seconds=int(os.environ.get('PERMANENT_SESSION_LIFETIME', 1800)))
...
if force_https:
    ...
    # Development mode - add headers without HTTPS redirect
    @app.after_request
...
class SecurityLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    event_type = db.Column(db.String(50), nullable=False)  # login_failed, user_created, etc.
...
def log_security_event(event_type, username=None, details=None):
    ...
    # Log to file
    logger.info(f"Security Event: {event_type} | User: {username} | IP: {ip_address} | Details: {details}")
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if session.get('role') != 'admin':
            ...
            log_security_event('access_denied', session.get('user'), 'Attempted to access admin route')
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            ...
            if not user.is_active:
                log_security_event('login_failed', username, 'Account deactivated')
...
def create_user():
    ...
    if request.method == 'POST':
        ...
        log_security_event('user_created', session.get('user'), f'Created user: {username} with role: {role}')
...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        log_security_event('user_updated', session.get('user'), f'Updated user: {user.username}')
...
def delete_user(user_id):
    ...
    log_security_event('user_deleted', session.get('user'), f'Deleted user: {username}')
...
def billing():
    ...
    if request.method == 'POST':
        ...
        if customer_mobile:
            ...
            clean_mobile = re.sub(r'[\s\-\(\)]', '', customer_mobile)
        ...
        try:
            ...
            flash('Invalid GST or discount value')
            return render_template('billing.html', tiles=tiles)
...
def init_db():
    ...
    if not User.query.first():
        # Use stronger default password (still should be changed immediately)
        ...
        log_security_event('admin_created', 'admin', 'Default admin user created')
